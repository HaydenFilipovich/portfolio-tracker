...
# --- AI Risk Brief prompts ---

RISK_BRIEF_SYSTEM = """\
You are a portfolio risk analyst. Given portfolio data and computed risk \
metrics, produce a concise risk brief.

Structure your response in exactly three sections:

**Portfolio Overview**
2-3 sentences summarizing the portfolio composition, total value, and overall \
risk posture.

**Key Risk Findings**
3-5 bullet points citing specific numbers from the data. Cover concentration \
risk, volatility, drawdown exposure, and tail risk (VaR/CVaR). Compare Sharpe \
and Sortino ratios to characterize the return/risk profile.

**Recommendations**
2-4 bullet points with specific, actionable suggestions. Reference the data \
to justify each recommendation.

Rules:
- Cite exact numbers from the provided data.
- Keep total response under 300 words.
- Do not use hedging language ("consider", "might", "could"). State findings \
directly.
- Do not add disclaimers about not being financial advice."""

RISK_BRIEF_USER_TEMPLATE = """\
## Portfolio Holdings
{holdings_table}
...
if not st.session_state.holdings:
    ...
    if not hist_prices:
        ...
        if len(daily_returns) < 2:
            ...
            # ==============================================================
            st.divider()
            st.header("AI Risk Brief")

            if anthropic is None:
                st.warning(
                    "Install the `anthropic` package to enable AI risk briefs: "
                    "`pip install anthropic`"
                )
            else:
                # -- Build the holdings markdown table --
                _total_mkt = df["Market Value"].sum()
                _h_lines: list[str] = []
                for _, _r in df.iterrows():
                    _mv = _r["Market Value"]
                    if _mv and _total_mkt > 0:
                        _h_lines.append(
                            f"| {_r['Ticker']} | {_mv / _total_mkt:.1%} "
                            f"| ${_mv:,.0f} |"
                        )
                _holdings_md = (
                    "| Ticker | Weight | Value |\n"
                ...
                # -- Build the stress-test markdown table --
                _s_lines = [
                    f"| {r['Scenario']} | {r['Portfolio Drop']} "
                    f"| ${r['Loss ($)']:+,.0f} |"
                    for r in stress_results
                ]
                _stress_md = (
                    "| Scenario | Portfolio Drop | Loss |\n"
                    "|----------|----------------|------|\n"
                    + "\n".join(_s_lines)
                )

                brief_context = RISK_BRIEF_USER_TEMPLATE.format(
                    holdings_table=_holdings_md,
                    ...
                    var_hist_dollar=f"{var_hist_dollar:,.0f}",
                    var_hist_pct=f"{var_hist_pct:.2%}",
                    cvar_hist_dollar=f"{cvar_hist_dollar:,.0f}",
                    cvar_hist_pct=f"{cvar_hist_pct:.2%}",
                    var_param_dollar=f"{var_param_dollar:,.0f}",
                    var_param_pct=f"{var_param_pct:.2%}",
                    cvar_param_dollar=f"{cvar_param_dollar:,.0f}",
                    cvar_param_pct=f"{cvar_param_pct:.2%}",
                    mc_horizon=horizon,
                    mc_sims=f"{n_sims:,}",
                    current_value=f"{total_value:,.0f}",
                    mc_mean=f"{mc_mean:,.0f}",
                    mc_median=f"{mc_median:,.0f}",
                    mc_5=f"{mc_5:,.0f}",
                    mc_95=f"{mc_95:,.0f}",
                    stress_table=_stress_md,
                )

                with st.expander("View prompt sent to Claude"):
                    st.caption("System prompt")
                    st.code(RISK_BRIEF_SYSTEM, language="text")
                    st.caption("User message (populated with your data)")
                    st.code(brief_context, language="markdown")
                ...
                if not api_key:
                    st.info(
                        "Enter an Anthropic API key to generate AI risk briefs."
                    )
                else:
                    if st.button("Generate Risk Brief", key="gen_brief"):
                    ...
                    if "risk_brief" in st.session_state:
